    ...
    request.on('data', (chunk) => {
        ...
        if (jsonData !== undefined) {
            ...
            if (score !== undefined &&
                ...
                axios.post('https://funkomemorygame-default-rtdb.firebaseio.com/.json', score)
                    .then(() => response.send('Score saved!'))
...
app.get('/scores', (request, response) => {
    axios.get('https://funkomemorygame-default-rtdb.firebaseio.com/.json')
        .then(res => response.send(res.data))
        .catch(() => response.send(JSON.stringify({ error: 'Error requesting scores' })));
});
